...
| 13 | 搜索"稻香"并播放                  | 检查search_history.json文件中searches数组是否有query为"烟雨"且action为"play"的记录 | task_13_check_search_and_play(search_query) |
...
| 17 | 漫游播放并设置播放场景为"欢快"           | 检查player_settings.json文件中strollMode.scene字段是否为"伪感" | task_17_check_stroll_scene_setting(scene_name) |
...
| 30 | 搜索一位歌手<br/>并播放MV                | 检查mv_playback.json文件中currentMV.mvId为指定MV且isPlaying为true | task_30_check_play_mv(mv_id) |
