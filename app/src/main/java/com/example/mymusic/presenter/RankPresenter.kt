...
class RankPresenter(
    ...
    override fun loadRankDetail(rankId: String) {
        ...
        try {
            ...
            // 生成100首歌曲（循环使用现有歌曲）
            val rankSongs = mutableListOf<RankContract.RankSongItem>()
            for (i in 1..100) {
                val songIndex = (i - 1) % allSongs.size
                val song = allSongs[songIndex]

                // 检查是否已收藏
                val isFavorite = DataLoader.isSongInPlaylist(context, "my_favorites", song.songId)
    ...
    override fun onFavoriteClick(songId: String) {
        try {
            // 获取当前收藏状态
            val isFavorite = DataLoader.isSongInPlaylist(context, "my_favorites", songId)

            if (isFavorite) {
                view.showSuccess("已在我喜欢的音乐中")
            } else {
                // 添加到我喜欢的音乐
                val success = DataLoader.addSongToPlaylist(context, "my_favorites", songId)
                if (success) {
