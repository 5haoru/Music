...
- [x] **我的页面完整实现**（简化版，参考网易云音乐设计）
  ...
  - [x] 统计信息（关注/粉丝/等级/听歌时长）- 粉丝数量动态更新，可点击进入粉丝列表
...
- [x] **关注页面完整实现**（SubscribeTab - 二级页面）
  - 从我的页面"关注"按钮进入
  - **顶部导航栏**（三栏标签切换：推荐/关注/粉丝）
  - 返回按钮和搜索按钮
  - **筛选栏**（按关注时间排序、类型筛选：全部/歌手/用户）
  - **聚合更新横幅**（显示多位艺人的新动态）
  - **关注列表**（用户头像、昵称、VIP标识、动态信息）
  - VIP徽章系统（SVIP金色、VIP蓝色、黑胶SVIP）
  - 活动状态显示（发布MV、发布动态等）
  - 推荐区域（底部推荐卡片）
  - **取消关注功能**（UnfollowDialog）
    - 点击列表项右侧"更多"按钮（三个点）弹出对话框
    - 提示文本："取消关注后对方将从你的关注列表中移除"
    - 取消关注按钮（PersonRemove图标 + "取消关注"文字）
    - Material3 AlertDialog组件
    - 取消关注后立即从列表中移除
    - 内存中实时更新，保持当前筛选状态
  - MVP架构（SubscribeContract、SubscribePresenter）
  - 9个关注数据（follow_items.json）
  - 类型筛选功能（艺术家/用户筛选）
  - 时间排序功能（按关注时间降序）
- [x] **粉丝页面完整实现** (FanTab - 二级页面)
  - 从我的页面用户头像下面的“粉丝”入口进入，添加了粉丝后修改我的页面的粉丝数量。
  - **顶部导航栏**：
    - 左侧返回箭头 + 页面标题“粉丝”。
  - **粉丝列表项**：
    - 头像区：圆形头像（左侧）。
    - 用户信息：
      - 昵称（突出显示）。
      - VIP等级标签（如“VIP·叁”“SVIP·叁”）。
      - 副文本（个性签名/点赞等动态信息，灰色字体）。
    - 操作按钮：右侧黄色“+关注”按钮，边框设计。
  - MVP架构（FanContract、FanPresenter）。
  - 数据模型（FanRecord）和数据文件（fan_items.json）。
  - `DataLoader.kt` 中添加 `loadFanItems` 方法。
